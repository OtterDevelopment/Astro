...
export default class DenySuggestion extends SlashCommand {
    constructor(client: BetterClient) {
        super("deny", client, {
            ...
            options: [
                {
                    name: "suggestion",
                    description: "The number of the suggestion to deny.",
                    type: "INTEGER",
                    required: true
                },
                {
    ...
    override async run(interaction: CommandInteraction) {
        ...
        return Promise.all([
                ...
                this.client.functions.generateSuccessMessage(
                    {
                        ...
                        description: `Find the suggestion you denied [here](${suggestionMessage.url}).`
                    },
                    [],
                    true
                )
            ...
            this.client.mongo
                ...
                .findOne({ guildId: interaction.guild!.id })
                .then(entry => {
                    if (!entry || (!entry.type.all && !entry.type.denied))
                        return;

                    const suggestionLogChannels = [
                        this.client.channels.cache.get(entry.type.all || ""),
                        this.client.channels.cache.get(entry.type.denied || "")
